...
$query = "SELECT * FROM physics  where subject='physics' order by id DESC LIMIT $start_from, $record_per_page ";
...
<body onload="clock()" >
   ...
   ?>
    ...
    <form  action="answerdata.php?ans=physicsanswer && subject=physics &&  quesno= <?php echo $page; ?>&& pid=<?php echo $id; ?> && pageu=<?php echo $page;?>" method="post" >
...
<div style="width:315px; position:fixed; left:1023px; bottom:175px; height:350px; border:2px solid black; float:right ">
 ...
 <?php
   ...
   $page_query = "SELECT * FROM physics where subject='physics' ORDER BY id DESC";
    ...
    {     
               ...
               $qr="select * from answer where userid='$usern' AND status='attempt'";
      $runn=mysqli_query($connect, $qr);
      while($rowr=mysqli_fetch_array($runn))
      {
          $statusid=$rowr['questionid'];
          if($i==$statusid)
          {
              
            echo "<a id='skip' style=' padding:8px 16px;  border:1px solid black; background:green; color:white; text-decoration: none; font-weight:bold; position:relative; top:15px; left:7px;' href='test.php?sub=physics && page=".$i."'>".$i."</a>";
...
else
{
            ...
            echo "<a id='skip' style=' padding:8px 16px;  border:1px solid black;  color:black; text-decoration: none; font-weight:bold; position:relative; top:15px; left:7px;' href='test.php?sub=physics && page=".$i."'>".$i."</a>";
         } 
 }
}
   ...
   $page_query = "SELECT * FROM physics where subject='physics' ORDER BY id DESC";
    ...
    if($page > 1)
    {
     ...
     echo "<a style=' padding:8px 16px; border:1px solid #ccc; font-weight:bold; text-decoration: none; position:relative; bottom:450px; right:150px; background:navy; color:white;' href='test.php?sub=physics && page=".($page-1)."'>Previous</a>";
    }
  
    if($page < $end_loop)
    {
     echo "<a  name='skip' style=' padding:8px 16px; border:1px solid #ccc; color:white; text-decoration: none; font-weight:bold; position:relative; bottom:450px; right:150px; background:navy; color:white;'  href='test.php?sub=physics && page=".($page + 1)." && skip=skip'> Skip</a>";
